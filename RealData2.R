...
###############################################real data 
secom <- read.csv("C:/Users/Acer/Desktop/My Document/Documents/Research/Projects/change points(SaMT)/secom.data", sep="")
data=secom[c(4,8,15,79)]           
n=nrow(data)
d=ncol(data)
x=(1:n)/n
for (i in 1:d){
...
a=2048*c(which(accessD(threshold,level=7)!=0)/2^7-2^(-8), which(accessD(threshold,level=8)!=0)/2^8-2^(-9), which(accessD(threshold,level=9)!=0)/2^9-2^(-10))
num1[4]=sum(diff(sort(a*(a<1024)+(2048-a)*(a>1024)))>8)+1
#######SaMT
bandwidth1=optbandwidth(y1,x,h1=c(10,15,20,25,30)/length(x),h2=c(0.05,0.1,0.15))
h=bandwidth1[1]
...
######Wavelet
yy=c(y2,y2[1566-(1:482)])
wavelet=wd(yy, filter.number=1, family="DaubExPhase")
threshold=threshold(wavelet, levels =1:(nlevelsWT(wavelet)-1), type = "hard", policy = "universal",dev = madmad)
a=2048*c(which(accessD(threshold,level=7)!=0)/2^7-2^(-8), which(accessD(threshold,level=8)!=0)/2^8-2^(-9), which(accessD(threshold,level=9)!=0)/2^9-2^(-10))
num2[4]=sum(diff(sort(a*(a<1024)+(2048-a)*(a>1024)))>8)+1
#######SaMT
bandwidth2=optbandwidth(y2,x,h1=c(10,15,20,25,30)/length(x),h2=c(0.05,0.1,0.15))
h=bandwidth2[1]
...
######Wavelet
yy=c(y3,y3[1566-(1:482)])
wavelet=wd(yy, filter.number=1, family="DaubExPhase")
threshold=threshold(wavelet, levels =1:(nlevelsWT(wavelet)-1), type = "hard", policy = "universal",dev = madmad)
a=2048*c(which(accessD(threshold,level=7)!=0)/2^7-2^(-8), which(accessD(threshold,level=8)!=0)/2^8-2^(-9), which(accessD(threshold,level=9)!=0)/2^9-2^(-10))
num3[4]=sum(diff(sort(a*(a<1024)+(2048-a)*(a>1024)))>8)+1
#######SaMT
...
######SaRa                 
num4[3]=sum(p.adjust(SARA(y4,h=10)$pV, "BH")<0.1)
######Wavelet
yy=c(y4,y4[1566-(1:482)])
wavelet=wd(yy, filter.number=1, family="DaubExPhase")
threshold=threshold(wavelet, levels =1:(nlevelsWT(wavelet)-1), type = "hard", policy = "universal",dev = madmad)
a=2048*c(which(accessD(threshold,level=7)!=0)/2^7-2^(-8), which(accessD(threshold,level=8)!=0)/2^8-2^(-9), which(accessD(threshold,level=9)!=0)/2^9-2^(-10))
num4[4]=sum(diff(sort(a*(a<1024)+(2048-a)*(a>1024)))>8)+1
#######SaMT
